...
namespace PlagiarismChecker
{
    public partial class frmMain : Form
    {
        ...
        private void ShowFilesInTreeView(List<TargetDocumentInfo> documents)
        {
            var root = new TreeNode();

            foreach (var document in documents)
            {
                var node = new TreeNode(Path.GetFileNameWithoutExtension(document.FilePath));
                root.Nodes.Add(node);
                foreach (var file in document.DocumentContentFiles)
                {
                    node.Nodes.Add(new TreeNode(Path.GetFileNameWithoutExtension(file.FilePath)));
                }
            }
        }
        ...
        private void OpenToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var ofdMain = new OpenFileDialog {Filter = "doc, docx文件|*.doc;*.docx|xls, xlsx文件|*.xls;*.xlsx|ppt, pptx文件|*.ppt;*.pptx|rar文件|*.rar|所有文件|*.*", Multiselect = true};
            ...
            if (ofdMain.ShowDialog() == DialogResult.OK)
            {
                foreach (var fileName in ofdMain.FileNames)
                {
                    ...
                    FileHelper.ConvertToDocx(sourceFiles.ToArray());
                    ...
                    {
                        tvwFiles.Nodes.Add(new TreeNode(a.DocumentName));
                    }
